...
module.exports = {
    ...
    */
    run: async (interaction) => {
        ...
        if (!interaction.member.permissions.has("BAN_MEMBERS")) return interaction.reply('Bạn không có quyền `BAN_MEMBERS`')
        if (!interaction.guild.me.permissions.has("BAN_MEMBERS")) return interaction.reply('Tôi không có quyền `BAN_MEMBERS`')
                ...
                if (data.ban !== 'No data') {
                    ...
                    if (bans.has(userid)) {
                        try {
                            ...
                            interaction.reply(`Đã gỡ cấm cho ${userid}\nXem thêm tại <#${data.ban}>`)
                            Channel.send({
                                embeds:
                                    [
                                        new MessageEmbed()
                            ...
                            interaction.reply('🛑 | Gặp lỗi khi unban user!')
                        }
                    } else {
                        interaction.reply('UserID không hợp lệ hoặc không bị cấm.')
                    }
                } else {
                    if (bans.has(userid)) {
                        try {

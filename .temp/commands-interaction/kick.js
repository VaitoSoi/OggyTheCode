...
module.exports = {
    ...
    */
    run: async (interaction) => {
        ...
        if (!interaction.member.permissions.has("KICK_MEMBERS")) return interaction.reply('Bạn không có quyền `KICK_USERS`')
        if (!interaction.guild.me.permissions.has("KICK_MEMBERS")) return interaction.reply('Tôi không có quyền `KICK_USERS`')
            ...
            if (data) {
                if (data.kick !== 'No data') {
                    ...
                    if (member.kickable) {
                        ...
                        interaction.reply(`Đã kick ${member.user.tag}`)
                        Channel.send(embed)
                    } else {
                        interaction.reply('Không thể kick thành viên ' + member.user.tag)
                    }
                } else {
                    if (member.kickable) {
                        if (interaction.options.getBoolean('dms') === true) member.send(embed)
                        ...
                        interaction.reply('Không thể đuổi thành viên ' + member.user.tag)
                    }
                }
            } else {
                if (member.kickable) {

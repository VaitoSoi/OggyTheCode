...
module.exports = {
    ...
    */
    run: async (interaction) => {
        ...
        if (action === 'show') {
            if (!data) return interaction.editReply('🛑 | Không phát data cho guild này!\n🟢 | Dùng lệnh `/config create` để tạo data!')
                    ...
                    if (channel.isText()) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                interaction.channel.send(`✅ | Đã khóa kênh <#${channel.id}>`)
                            } catch (error) {
                                if (type === 'livechat') interaction.channel.send(`🟡 | Vui lòng khóa kênh <#${channel.id}>.\n▶ | Lý do: Tính năng chat trong để gửi tin nhắn đã bị xóa!`)
                                else if (type === 'status') interaction.channel.send(`🟡 | Vui lòng khóa kênh <#${channel.id}>.\nLý do: Tránh trôi tin nhắn!`)
                            }
                        } catch (e) {

    ...
    */
    run: async (interaction) => {
        ...
        if (data) {
            ...
            if (!role) return interaction.editReply('Role được cho không hợp lệ!')
        } else {
            role = interaction.guild.roles.cache.find(r => r.name.toLowerCase() === 'muted')
        }
        if (!interaction.memberPermissions.has("MANAGE_ROLES")) return interaction.editReply('Bạn không có quyền `MANAGE_ROLES`')
        if (!interaction.guild.me.permissions.has("MANAGE_ROLES")) return interaction.editReply('Tôi không có quyền `MANAGE_ROLES`')
        ...
        if (Member.roles.cache.has(role.id)) return interaction.editReply(`${Member.displayName} đã bị mute từ trước!`)
            ...
            if (data) {
                if (data.mute !== 'No data') {
                    ...
                    if (interaction.options.getBoolean('dms') === true) Member.send(embed)
                        .catch(error => { })
                    interaction.editReply('Đã mute ' + Member.user.tag)

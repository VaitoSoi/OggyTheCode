    ...
    run: async (interaction) => {
        ...
        setchannel.finOne({ guilid: interaction.guild.id }, async (err, data) => {
            ...
            if (data) {
                if (data.ban !== 'No data') {
                    if (member.bannable) {
                        member.send(embed).catch(error => interaction.editReply({ content: 'Không thể gửi tin nhắn cho thành viên được mention(tags)!' }))
                            .then(m => member.ban({ reason }))
                        interaction.editReply({ embeds: [embed] })
                        interaction.client.channels.cache.get(data.ban).send({ embeds: embed })
                    } else {
                        interaction.editReply({ content: 'Không thể cấm thành viên được mention(tag)!' })
                    }
                } else {
                    if (member.bannable) {
                        ...
                        interaction.editReply('Không thể cấm thành viên được mention(tag)!')
                    }
                }
            } else {
                if (member.bannable) {
                    ...
                    interaction.editReply({ content: 'Không thể cấm thành viên được mention(tag)!', ephemeral: ephemeral })
                }
            }
        })
    }

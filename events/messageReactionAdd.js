...
module.exports = {
    ...
    async run(reaction, user) {
        ...
        if (reaction.emoji.name === '🔁') {
            if (!data.config.message.status
                || data.config.message.status !== reaction.message.id
                || user.id === reaction.client.user.id) return
            ...
            if (!role) return reaction.users.remove(user.id)
            else {
